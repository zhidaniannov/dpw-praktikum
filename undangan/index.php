...
<head>
  ...
  <!-- Tambahan Favicon agar tidak error 404 -->
...
<body>
  ...
  <main id="mainContent" class="main visible" aria-hidden="false">
    ...
    <section id="rsvp" class="section section--rsvp fade-in">
      <div class="section__inner">
        ...
        <!-- Changed IDs to bypass original script.js fetch interference -->
        <form id="rsvpFormReal" class="form mx-auto px-4 max-w-xl" novalidate>

          <!-- Name -->
          <div class="form__group">
            <label for="rsvpName" class="form__label">Nama Lengkap</label>
            <input type="text" id="rsvpName" name="name" class="form__input px-4 py-3 rounded-md"
              placeholder="Masukkan nama lengkap Anda" maxlength="25" required />
          </div>
          ...
          <div class="form__group">
            ...
            <div class="form__radio-group flex flex-col md:flex-row md:items-center md:gap-6">
              <label class="form__radio">
                <input type="radio" name="option" value="H" required />
                <span class="form__radio-box"></span>
                <span>Insya Allah Hadir</span>
              </label>
              <label class="form__radio">
                <input type="radio" name="option" value="TH" />
                <span class="form__radio-box"></span>
          ...
          <!-- Guest count -->
          <div class="form__group" id="guestCountGroupReal">
            <label for="guestCount" class="form__label">Jumlah Tamu (pax)</label>
            <select id="guestCount" name="member" class="form__select px-4 py-3 rounded-md">
          ...
          <button type="submit" id="rsvpSubmitBtnReal"
            class="btn btn--primary bg-[#5c4b3b] text-[#fdf6e3] hover:bg-[#2b2b2b] py-3 px-4 rounded-md w-full">
            Kirim Konfirmasi
          </button>
  ...
  <!-- RSVP Custom Overwrite Script -->
  <script>
    document.addEventListener('DOMContentLoaded', function() {
        // Toggle opacity for guest count
        document.querySelectorAll('input[name="option"]').forEach(radio => {
            radio.addEventListener('change', function() {
                const group = document.getElementById('guestCountGroupReal');
                if(this.value === 'H') {
        ...
        // Form Submit Handler
        document.getElementById('rsvpFormReal').addEventListener('submit', function(e) {
            e.preventDefault();
            
            const form = this;
            const submitBtn = document.getElementById('rsvpSubmitBtnReal');
            const nameVal = form.name.value.trim();
            const optionEl = form.querySelector('input[name="option"]:checked');
            const memberVal = form.member.value;

            if (!nameVal) {
                if(typeof showToast === 'function') showToast('Mohon masukkan nama lengkap Anda.', 'error');
                return;
            }
            if (!optionEl) {
                if(typeof showToast === 'function') showToast('Mohon pilih konfirmasi kehadiran Anda.', 'error');
                return;
            }

            const body = new FormData();
            body.append('name', nameVal);
            body.append('option', optionEl.value);
            if (optionEl.value === 'H') {
                body.append('member', memberVal);
            }

            submitBtn.disabled = true;
            submitBtn.textContent = 'Mengirim...';

            // PASTIKAN NAMA FILE DI SINI BENAR: rsvp_action.php
            fetch('../rsvp_action.php', { method: 'POST', body: body })
            .then(res => res.text()) // Ambil sebagai teks dulu untuk menangkap error PHP
            .then(text => {
                try {
                    const data = JSON.parse(text);
                    if (data.success) {
                        const msg = optionEl.value === 'H'
                            ? '✅ Terima kasih, ' + nameVal + '! Kehadiran Anda untuk ' + memberVal + ' orang telah tercatat. 🎉'
                            : '💌 Terima kasih, ' + nameVal + '. Doa Anda sangat berarti bagi kami.';
                        if(typeof showToast === 'function') showToast(msg, 'success');
                        form.reset();
                        document.getElementById('guestCountGroupReal').style.opacity = '1';
                        document.getElementById('guestCountGroupReal').style.pointerEvents = 'auto';
                    } else {
                        if(typeof showToast === 'function') showToast(data.message || 'Terjadi kesalahan.', 'error');
                    }
                } catch (e) {
                    console.error("RESPONSE ERROR DARI SERVER:", text);
                    if(typeof showToast === 'function') showToast('Server Error PHP. Tekan F12 lalu cek tab Console.', 'error');
                }
            })
            .catch(err => {
                console.error("NETWORK ERROR:", err);
                if(typeof showToast === 'function') showToast('Koneksi terputus atau file tidak ditemukan.', 'error');
            })
            .finally(() => {
  ...
  <button id="soundToggle" class="btn btn--primary" aria-pressed="false" title="Mute / Unmute"
    style="position:fixed;right:1rem;bottom:1rem;z-index:120;">🔈</button>
